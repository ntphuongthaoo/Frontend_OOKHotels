    ...
    <div class="room-detail-container" v-if="room && room.CUSTOM_ATTRIBUTES">
      ...
      <div class="image-gallery" v-if="room.IMAGES && room.IMAGES.length > 0">
        <div class="main-image-container">
          <img :src="room.IMAGES[mainImageIndex]" alt="Room Image" class="main-image" />
        </div>
      ...
      <div class="room-info">
        ...
        <div class="info-item"><strong>Số phòng:</strong> {{ room.ROOM_NUMBER }}</div>
        <div class="info-item"><strong>Loại phòng:</strong> {{ room.TYPE }}</div>
        <div class="info-item"><strong>Diện tích:</strong> {{ room.CUSTOM_ATTRIBUTES.area }} m²</div>
        <div class="info-item"><strong>Giá mỗi đêm:</strong> {{ room.PRICE_PERNIGHT.toLocaleString() }} VND</div>
        <div class="info-item"><strong>Loại giường:</strong> {{ room.CUSTOM_ATTRIBUTES.bedType }}</div>
        <div class="info-item"><strong>Tầm nhìn:</strong> {{ room.CUSTOM_ATTRIBUTES.view }}</div>
        <div class="info-item"><strong>Số người:</strong> {{ room.CUSTOM_ATTRIBUTES.number_of_people }} người</div>
        <div class="info-item"><strong>Tiện nghi:</strong> {{ room.CUSTOM_ATTRIBUTES.amenities }}</div>
        <div class="info-item"><strong>Mô tả:</strong> {{ room.DESCRIPTION }}</div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchRoomReviews() {
        const roomId = this.$route.params.id;
        try {
          const response = await axiosClient.get(`/reviews/getReviewsByRoomId/${roomId}`);
